...
Polymer({
  _template: html`
      ...
      <div id="wrapper" class="inline--flex " style="height:100%;width:100%">
        <px-vis-svg-canvas class="inline--flex flex__item--msfix" width="[[width]]" height="[[height]]" margin="[[margin]]" svg="{{svg}}" px-svg-elem="{{pxSvgElem}}" canvas-context="{{canvasContext}}">
        </px-vis-svg-canvas>

        <template is="dom-if" if="[[!renderToCanvas]]">
          <px-vis-clip-path svg="[[layer.1]]" width="[[width]]" height="[[height]]" margin="[[margin]]" series-clip-path="{{seriesClipPath}}">
          </px-vis-clip-path>
        </template>
          ...
          <template is="dom-if" if="[[_limitSeries(index,navSeriesLimit)]]">
            <template is="dom-if" if="[[_chartTypeLine(item,completeSeriesConfig)]]" restamp="">
              <template is="dom-if" if="[[renderToCanvas]]" restamp="">
                <px-vis-line-canvas id="lineCanvas" canvas-context="[[canvasContext]]" series-id="[[item]]" chart-data="[[chartData]]" complete-series-config="[[completeSeriesConfig]]" x="[[x]]" y="[[y]]" domain-changed="[[domainChanged]]" show-gaps="[[showGaps]]" width="[[width]]" height="[[height]]" margin="[[margin]]" clip-path="">
                </px-vis-line-canvas>
              </template>
              <template is="dom-if" if="[[!renderToCanvas]]" restamp="">
                <px-vis-line-svg svg="[[layer.1]]" series-id="[[item]]" chart-data="[[chartData]]" complete-series-config="[[completeSeriesConfig]]" x="[[x]]" y="[[y]]" clip-path="[[seriesClipPath]]" domain-changed="[[domainChanged]]" muted-series="[[mutedSeries]]">
                </px-vis-line-svg>
              </template>
            </template>
            <template is="dom-if" if="[[_chartTypeScatter(item,completeSeriesConfig)]]" restamp="">
              <template is="dom-if" if="[[renderToCanvas]]" restamp="">
                <px-vis-scatter-canvas canvas-context="[[canvasContext]]" series-id="[[item]]" chart-data="[[chartData]]" complete-series-config="[[completeSeriesConfig]]" width="[[width]]" height="[[height]]" margin="[[margin]]" x="[[x]]" y="[[y]]" clip-path="" domain-changed="[[domainChanged]]" marker-size="2">
                </px-vis-scatter-canvas>
              </template>
              <template is="dom-if" if="[[!renderToCanvas]]" restamp="">
                <px-vis-scatter svg="[[layer.1]]" series-id="[[item]]" chart-data="[[chartData]]" complete-series-config="[[completeSeriesConfig]]" marker-size="2" x="[[x]]" y="[[y]]" clip-path="[[seriesClipPath]]" domain-changed="[[domainChanged]]" muted-series="[[mutedSeries]]">
                </px-vis-scatter>
              </template>
            </template>
          </template>
        ...
        <px-vis-axis id="xAxis" svg="[[layer.0]]" axis="[[x]]" axis-id="x" axis-type="[[xAxisType]]" height="[[height]]" width="[[width]]" margin="[[margin]]" orientation="bottom" prevent-series-bar="" complete-series-config="[[completeSeriesConfig]]" domain-changed="[[domainChanged]]">
        </px-vis-axis>
        <px-vis-brush svg="[[layer.2]]" axis="[[x]]" height="[[height]]" width="[[width]]" margin="[[margin]]" domain-changed="[[domainChanged]]" chart-domain="[[chartDomainX]]" selected-domain="{{selectedDomain}}" gradient-colors="[[gradientColors]]">
        </px-vis-brush>
      </div>
`,
