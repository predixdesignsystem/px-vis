...
Polymer({
  _template: html`
    <div class="outline">
      ...
      <px-vis-scale x-axis-type="time" y-axis-type="linear" complete-series-config="[[completeSeriesConfig]]" data-extents="[[dataExtents]]" width="[[width]]" height="[[height]]" margin="[[margin]]" chart-data="{{chartData}}" domain-changed="{{domainChanged}}" x="{{x}}" y="{{y}}" selected-domain="[[selectedDomain]]">
      </px-vis-scale>
      <px-vis-area-svg svg="[[svg]]" complete-series-config="[[completeSeriesConfig]]" chart-data="[[_returnStack(chartData)]]" x="[[x]]" y="[[y]]" domain-changed="[[domainChanged]]">
      </px-vis-area-svg>
    </div>
`,
