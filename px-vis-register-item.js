...
Polymer({
  _template: html`
    ...
    <span id="regItem" class\$="[[_baseClasses]] register-item--min-width" name\$="R[[seriesKey]]">
      <span id="regWithoutMenu" class\$="flex [[_itemMutedToStart]]" style="width:100%;">
        <span class="seriesMarker flex">
          <span class="seriesMarkerIcon" style\$="[[_calcDashPattern(_itemColor, _dashPattern, _negativeColor)]];" name\$="R[[seriesKey]]">&nbsp;</span>
        </span>
        <span class\$="[[_wrapperClass]]" style="width:100%;">
          ...
          <div class="seriesData flex__item--msfix" name\$="R[[seriesKey]]">
            [[_displayedData]]
          </div>
        </span>
      </span>
      <template id="menuTemplate" is="dom-if" if="[[_hasDynamicMenu]]">
        <px-vis-dynamic-menu id="menu" class\$="[[_getDynamicMenuClass(type)]]" additional-detail="[[seriesKey]]" dynamic-menu-config="[[dynamicMenuConfig]]">
        </px-vis-dynamic-menu>
      </template>
    </span>
    <template is="dom-if" if="[[_isOfType(xAxisType, 'linear')]]">
      <px-number-formatter-no-display id="xFormatter" value="[[_returnValue(data, 'x')]]" format="[[numberFormat]]" culture="[[numberFormatCulture]]" currency\$="[[numberFormatIsCurrency]]" default-currency-format="[[numberFormatCurrency]]" zero-format="[[numberFormatZero]]" formatted-value="{{_formattedX}}">
      </px-number-formatter-no-display>
    </template>
    <px-number-formatter-no-display id="yFormatter" value="[[_returnValue(data, 'y')]]" format="[[numberFormat]]" culture="[[numberFormatCulture]]" currency\$="[[numberFormatIsCurrency]]" default-currency-format="[[numberFormatCurrency]]" zero-format="[[numberFormatZero]]" formatted-value="{{_formattedY}}">
    </px-number-formatter-no-display>
`,
